...
- [Organizing Config](#organizing-config)
...
This realization leads to documenting the codes with comments (_the grey_) and it obviously adds more lines. There after the thoughts lead to modular config, splitting up of files in sensible manner and to import from one stop.
...
I won't introduce about org-mode, please do check [here](https://orgmode.org), hopefully it gives you an idea (I mean you should have read whole documentation/manual there). Additionally here is a [Wikipedia reference](https://en.wikipedia.org/wiki/Org-mode) which is enough to introduce.

The concept of [Literate programming](https://en.wikipedia.org/wiki/Literate_programming) is having the user's natural language written down in correspondence to code blocks. It adds human-ness to the code, and the instructions we need to understand. Over technical work, it can reduce the usage of manual pages and documentation, wait, that is not a good idea to avoid them, but for something to understand that we wrote ourselves, literate programming helps to add notes.

The concept is fairly like maintaining a notebook or documentation guide itself, but it also comes with its power.

Org-mode has tons of features that helps in various categories. Prime use is for maintaining GTD, Notes, Latex, Journaling... _infinity?_
...
That's where I decided to move back all to literate config. I came across [rasendubi](https://github.com/rasendubi/dotfiles) dotfiles and he also used Literate org config for both Nixos and Emacs. Eventually I did too, here is the [repo](https://github.com/idlip/d-nix).
...
As I bestowed this in practice, I noticed that flow of thoughts and expression of it at that time is the crucial thing. Again, for a thought to come and just vanish, its a haze. Even merely _sixty seconds_ is enough to forget. Org could also fill that void, but just capturing the running thought at that time, and later to expand on it.
...
In a recent snippet on [Organize File]({{< relref "organize-file" >}}), I was about falling out on maintaining notes. From that scenario also the lesson learned is that, more than maintaining notes, the actual flow of thought and feeling at right time is most important and beats the best organizer anywhere out there. I mean, the goal of note-taking or writing down should be to transcribe the thoughts and knowledge in a approachable manner.
